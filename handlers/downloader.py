...
def get_height_width(link: str, Q: str):
    ...
    if link.endswith("ankul60"):
        ...
        if "m3u8" in url:
            ...
            cook = "cooks.txt"
            YTDLP = f'yt-dlp -i --no-check-certificate {headers} -f "{YTF}" --no-warning "{url}" --cookies "{cook}" -o "%(resolution)s"'
            wXh = get_link_atributes().get_wxh(YTDLP)
            return wXh
        elif "youtu" in url:
            YTDLP = f'yt-dlp -i --no-check-certificate {headers} -f "{YTF}" --no-warning "{url}" --progress -o "%(resolution)s"'
            wXh = get_link_atributes().get_wxh(YTDLP)
            return wXh
    else:
        YTDLP = f'yt-dlp -i --no-check-certificate {headers} -f "{YTF}" --no-warning "{url}" --progress --remux-video mp4 -o "%(resolution)s"'
        wXh = get_link_atributes().get_wxh(YTDLP)
        return wXh
